...
const sampleProducts = [
  {
    id: 1,
    name: "Berry Smoothie & juice mixed with carbon hiper",
    category: "Smoothies",
  },
...
export default function ManagePage() {
  ...
  function selectAll(select) {
    ...
    for (let index = 0; index < availableProducts.length; index++) {
      ...
      setForm((s) =>
        !select ? {...s, products: []} : s.products.includes(element.id)
          ? { ...s, products: [...s.products] }
          : { ...s, products: [...s.products, element.id] }
      );
    }
  }
...
const TitleBar = ({ isEditing, setIsEditing, setDialogOpen }) => {
  return (
...
const PageInfo = ({ isEditing, form, updateField }) => {
  return (
  ...
  return (
    <div className="bg-white dark:bg-slate-800 max-h-fit rounded-xl p-4 border shadow-sm">
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        {["Smoothies", "Juice", "Whole Foods", "Groceries"].map((c) => (
          <button
            key={c}
...
const FooterArea = ({ isEditing, form, savePage, setForm, setIsEditing }) => {
  return (
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <div className="relative max-w-4xl w-full bg-white rounded-xl shadow-xl overflow-hidden">
        ...
        <div className="p-4 pb-2 border-b">
          ...
          {dialogTab === "select" && isEditing && (<div onClick={select_all} className="cursor-pointer flex items-center w-full text-sm gap-1 mt-3">
